...
func (h *BrokerHandler) BindServiceInstance(w http.ResponseWriter, r *http.Request, vars map[string]string) {
	...
	err = custom_metrics_cred_helper.CreateCustomMetricsCredential(body.AppID, h.policydb, custom_metrics_cred_helper.MaxRetry)
	if err != nil {
	...
	handlers.WriteJSONResponse(w, http.StatusCreated, nil)
}
