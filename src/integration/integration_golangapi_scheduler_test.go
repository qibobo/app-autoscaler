	...
	Describe("When offered as a service", func() {
		...
		Context("Scheduler is down", func() {
			...
			Context("Delete policy", func() {
				...
				// 		policyStr = readPolicyFromFile("fakePolicyWithSchedule.json")
				// 		doAttachPolicy(appId, policyStr, http.StatusCreated, components.Ports[GolangAPIServer], httpClientForPublicApi)
				// 	})
		...
		Describe("Delete Policies", func() {
			...
			// 			doDetachPolicy(appId, http.StatusNotFound, `{"error":"No policy bound with application"}`, components.Ports[GolangAPIServer], httpClientForPublicApi)
			// 		})
			// 	})
